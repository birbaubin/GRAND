...
class Graph(object):
    ...
    def __init__(self, nodes, with_fixed_edges=False):
        ...
        if with_fixed_edges:
            self.adj_matrix = csr_matrix((self.size, self.size), dtype=np.int8)
        else:
            self.adj_matrix = csr_matrix((self.size, self.size), dtype=np.int8)
            self.adj_matrix[:] = 2
    ...
    def neighbors(self, n1):
        return self.adj_matrix.rows[n1]
    ...
    def edges(self):
        list = []
        non_zero_indices = self.adj_matrix.nonzero()
    ...
    def stats(self):
        ...
        num_present_edges = len(self.edges())
        num_unknown_edges = self.size * (self.size - 1) / 2 - num_present_edges - num_absent_edges

        return num_absent_edges, num_present_edges, num_unknown_edges

    def copy(self):
        copy =  Graph(self.nodes)
        copy.adj_matrix = self.adj_matrix.copy()
        return copy
    ...
    def from_adj_matrix(adj_matrix, with_fixed_edges=False):
        ...
        graph.adj_matrix = lil_matrix(adj_matrix)
        return graph

    @staticmethod
    def from_npy(filepath):
        adj_matrix = np.load(filepath, allow_pickle=True)
        return Graph.from_adj_matrix(lil_matrix(adj_matrix), with_fixed_edges=True)
