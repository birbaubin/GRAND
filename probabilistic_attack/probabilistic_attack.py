...
def A_based_completion(gstar, k, A, method="common_neighbors"):
    gstar_copy = gstar.copy()
    unknown_edges = set(gstar_copy.unknown_edges())
    ...
    if method == "common_neighbors":
        ...
        predictions = [((edge[0], edge[1]), A[edge[0]][edge[1]]) for edge in sorted_indices if (edge[0], edge[1]) in unknown_edges]
        ...
        for edge in unknown_edges:
            n1, n2 = edge
            scores.append(A[n1][n1] * A[n2][n2])

        predictions = list(zip(unknown_edges, scores))
    ...
    pbar = tqdm(total = k, desc="A completion with metric " + method)
    while additions < k and i < len(predictions):        
        edge, score = predictions[i]
        ok = True
        n1, n2 = edge
