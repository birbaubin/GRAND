...
class RevisitedSpectral:
    ...
    def sanity_check_ultimate(self):
        ...
        for i, j in more_slots:
            neighbors = self.Gstar.common_neighbors((i, j))
            for n in neighbors:
                if self.last_G.adj_matrix[i, n] == 2:
                    self.Gstar.adj_matrix[i, n] = 2
                    self.Gstar.adj_matrix[n, i] = 2
                    modifs += 1
                if self.last_G.adj_matrix[j, n] == 2:
                    self.Gstar.adj_matrix[j, n] = 2
                    self.Gstar.adj_matrix[n, j] = 2
                    modifs += 1

        for i, j in less_slots:
            neighbors = self.Gstar.common_neighbors((i, j))
            non_common_neighbors = set(self.Gstar.nodes) - neighbors
