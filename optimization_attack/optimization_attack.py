...
def similarity_based_completion(gstar, k, A, method="common_neighbors"):
    ...
    unknown_edges = set(gstar.unknown_edges())
    predictions = {edge: gstar.link_prediction_scores(method=method, edge=edge) for edge in unknown_edges}
    ...
    for i in tqdm(range(k)):
        ...
        gstar_current = gstar.copy()
        edge, score = predictions[i]
        gstar_current.add_edge(edge)
        gstar_current.fix_edges()
        current_A = np.dot(gstar_current.adj_matrix, gstar_current.adj_matrix.T)
        distance = np.sum(A - current_A)
        if distance > max_distance:
            max_distance = distance
            selected_edge = edge

    gstar_copy.add_edge(selected_edge)
