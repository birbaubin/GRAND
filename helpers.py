...
def reconstruction_error(prediction, groundtruth):
    ...
    return np.sum(np.abs(Gsquare - Gsquare_pred)) / np.sum(Gsquare)
...
def log_graph_stats_with_defense(graph1_prob, common_prob, expe, attack_type, proba_params, optim, iter_number, time, file_name, prediction, groundtruth, dp_graph, epsilon):
    stats = prediction.stats()
    TP, FP, TN, FN = ROC_stats(prediction, groundtruth)
    RAE = (FP + FN) / (TP + FN)
    
    TP_dp, FP_dp, TN_dp, FN_dp = ROC_stats(dp_graph, groundtruth)
    RAE_dp = (FP_dp + FN_dp) / (TP_dp + FN_dp)
    
    CNE = reconstruction_error(prediction, groundtruth)
    CNE_dp = reconstruction_error(dp_graph, groundtruth)
    
    with open(file_name, "a+") as f:
        f.write(f"{expe},{attack_type},{optim},{proba_params[0]},{proba_params[1]},{proba_params[2]},{graph1_prob},{common_prob},{iter_number},{epsilon},{stats[0]},{stats[1]},{stats[2]},{RAE_dp},{RAE},{CNE_dp},{CNE},{time}\n")
        f.close()
    return stats[0], stats[1], stats[2]
...
def community_init(mat0,mat0_graph,epsilon,nr,t=1.0):
    ...
    mat_new_par = comm.best_partition(mat_one_level_graph,resolution=t)
