...
for expe in range(number_of_experiments):
    for graph1_prop, common_prop in product(graph1_props, common_props):
        ...
        while continue_deterministic:
            ...
            reconstructed_graph, number_modifs_rectangle = rectangle_attack(reconstructed_graph, A, np.linspace(1, 15, 1, dtype=int, endpoint=True))
            ...
            for (u, v) in reconstructed_graph.edges():
                if dataset.adj_matrix[u, v] == 0:
                    print(f"Edge {u} - {v} is not in the original graph")
